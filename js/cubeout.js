...
function resize() {
  if (window.innerWidth < 500 && window.innerHeight > window.innerWidth) {
    alert("Please Rotate For Better Gamming Experience");
  }
}
...
class Swipe {
  ...
  handleTouchRotateUp(evt) {
    if (
      ...
      !this.yDown ||
      this.xDown >= window.innerWidth / 2 ||
      this.yDown > window.innerHeight * 0.65 ||
      game_over_flag == true
    ) {
      return 0;
    }
  ...
  handleTouchRotateDown(evt) {
    if (
      ...
      console.log("Upper Side, y : ", this.yDown);
      return 0;
    }

    console.log("Lower Side, y : ", this.yDown);
...
var SLOW_ANIM_DURATION = 550;
var MED_ANIM_DURATION = 70;
var FAST_ANIM_DURATION = 5;
      ...
      for (var x = 0; x < width; ++x) {
        ...
        switch (type) {
          ...
          case 3:
            if (z >= 0)
              layers[z][y][x] =
                (depth - z) * (width - x + (height - y) > z ? 0 : 1);
            break;
          case 4:
        ...
        for (i = 0; i < N_ELEMENTS; ++i) {
          if (!PAUSE_WORMS)
            c =
              ...
              h +
              ",90%," +
              ((40 * (PIT_DEPTH - PAUSE_ELEMENTS[i][2])) / PIT_DEPTH).toFixed(
                0
              ) +
              "%)";
          else {
...
function game_loop(canvas, ctx) {
  ...
  // animate
  STATE.current_x =
    STATE.start_x + STATE.progress * (STATE.new_x - STATE.start_x);
  STATE.current_y =
    STATE.start_y + STATE.progress * (STATE.new_y - STATE.start_y);
  STATE.current_z =
    STATE.start_z + STATE.progress * (STATE.new_z - STATE.start_z);
  ...
  if (STATE.progress >= 1) {
    ...
    STATE.current_matrix = matmult(
      get_combined_rotmatrix(angles),
      STATE.start_matrix
    );
  }
...
function save_settings() {
  var tmp =
    SET + ":" + PIT_WIDTH + ":" + PIT_HEIGHT + ":" + PIT_DEPTH + ":" + SPEED;
...
function refresh_column() {
  ...
  for (i = 0; i < COUNTS.length; ++i) {
    ...
    if (COUNTS[i] != undefined && COUNTS[i] > 0) {
      ...
      var c2 =
        "hsla(" + c[0] + "," + c[1] + "%," + (c[2] - 10) + "%," + c[3] + ")";
      var c1 =
        "hsla(" + c[0] + "," + c[1] + "%," + (c[2] - 30) + "%," + c[3] + ")";
...
function setRotateSpeed(spd) {
  if (spd === "slow") ANIM_DURATION = SLOW_ANIM_DURATION;
  else if (spd === "fast") {
...
function showScoreUIAuthor() {
  ...
  if (
    STATE.score > 0 &&
    (typeof HIGHSCORE[USERNAME] === "undefined" ||
      HIGHSCORE[USERNAME].score < STATE.score)
  ) {
    HIGHSCORE[USERNAME] = {
      mode: dh,
      score: STATE.score,
...
function validateUsername(event) {
  ...
  if (
    (validUsernameRegex.test(usernameInp.val()) &&
      usernameInp.val().length > 0 &&
      usernameInp.val().length < 20) ||
    !usernameInp.val().length
  ) {
...
async function saveScoreByRecentlyInsertedUsername(
  usernameSubmitBtn,
  usernameInp
) {
...
async function saveScorebyNewlyInsertedUsername(
  usernameSubmitBtn,
  usernameInp
) {
  var res = await updateScore();
...
function askForEmptyUsernameInsertion(usernameSubmitBtn, usernameInp) {
  if (
    confirm(
      "Oops! Username already exists. Do you want to save an empty username?"
    )
  ) {
    game.playerName = "";
    usernameInp.val("");
...
function emptyUsernameInsertionProcess(usernameSubmitBtn, usernameInp) {
  confirm(
    "Ouch! Empty name inserted. Do you want to save your score by recently inserted name?"
  )
    ? saveScoreByRecentlyInsertedUsername(usernameSubmitBtn, usernameInp)
    : saveScoreByEmptyUsername(usernameSubmitBtn, usernameInp);
}

async function nonEmptyUsernameInsertionProcess(
  usernameSubmitBtn,
  usernameInp
) {
  ...
  res.status != "success"
    ? saveScorebyNewlyInsertedUsername(usernameSubmitBtn, usernameInp)
    : askForEmptyUsernameInsertion(usernameSubmitBtn, usernameInp);
}
...
function showScoreUI() {
  ...
  game.playerName == ""
    ? emptyUsernameInsertionProcess(usernameSubmitBtn, usernameInp)
    : nonEmptyUsernameInsertionProcess(usernameSubmitBtn, usernameInp);
}
